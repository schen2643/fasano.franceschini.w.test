...
std::vector<double> testStatistics(const MatrixT& M,
                                   std::vector<double> w, /** sc changed here **/
                                   std::size_t n1,
                                   std::size_t n2,
                                   bool shuffle,
                                   std::mt19937& prng,
    ...
    double d1 = 0;
    double d2 = 0;
    // double w1 = std::accumulate(w.begin(), w.begin()+n1, 0LL); /** sc changed here **/
    // double w2 = std::accumulate(w.begin()+n1, w.begin()+n1+n2, 0LL);
    double w1 = 0; /** sc changed here **/
    double w2 = 0;
    for (int i = 0; i < n1; ++i) {
        w1 += w[i];
    }
    ...
    if (method == 'r') {
        // Build range trees
        std::vector<RTree> trees = buildRangeTrees<MatrixT>(M, w, n1, n2, s); /** sc changed here **/

        // Compute test statistic using first sample as origins
        for (std::size_t i = 0; i < n1; ++i) {
            d1 = std::max(d1, rangeDistance(trees[0], trees[1], w1, w2, getRow<MatrixT>(M, s[i])));
        }

        // Compute test statistic using second sample as origins
        for (std::size_t i = 0; i < n2; ++i) {
            d2 = std::max(d2, rangeDistance(trees[0], trees[1], w1, w2, getRow<MatrixT>(M, s[i + n1])));
        }
    } else {
        // Compute test statistic using first sample as origins
        for (std::size_t i = 0; i < n1; ++i) {
...
std::vector<double> testStatistics(const MatrixT& M,
    ...
    if (method == 'r') {
        ...
        // Compute test statistic using first sample as origins
        for (std::size_t i = 0; i < n1; ++i) {
            d1 = std::max(d1, rangeDistance(trees[0], trees[1], w1, w2, getRow<MatrixT>(M, s[i]))); /** sc changed here **/
        }

        // Compute test statistic using second sample as origins
        for (std::size_t i = 0; i < n2; ++i) {
            d2 = std::max(d2, rangeDistance(trees[0], trees[1], w1, w2, getRow<MatrixT>(M, s[i + n1]))); /** sc changed here **/
        }
    } else {
        // Compute test statistic using first sample as origins
        for (std::size_t i = 0; i < n1; ++i) {
...
std::vector<double> testStatistics(const MatrixT& M,
    ...
    std::mt19937 prng;
    return testStatistics<MatrixT>(M, w, r1, r2, false, prng, method);
}
...
std::vector<double> testStatistics(const MatrixT& M,
                                   ...
                                   std::mt19937& prng,
                                   char method) {
    return testStatistics<MatrixT>(M, w, r1, r2, true, prng, method);
}
...
NumericVector ffTestStatistic(const NumericMatrix& S1,
                              ...
                              std::vector<double> W, /** sc changed here **/
                              char method) {
    return wrap(testStatistics<NumericMatrix>(rbind(S1, S2), W, S1.nrow(), S2.nrow(), method));
}
...
unsigned int permutationTest(const NumericMatrix& S1,
    ...
    double Z = testStatistics<NumericMatrix>(S, W, r1, r2, method)[2]; /** sc changed here **/
    ...
    for (int i = 0; i < nPermute; ++i) {
        double z = testStatistics<NumericMatrix>(S, W, r1, r2, prng, method)[2]; /** sc changed here **/
        pval += (z >= Z);
        p.step();
    }
    p.finalize();
...
struct PermutationTest : public RcppParallel::Worker {
    ...
    // Constructors
    PermutationTest(const NumericMatrix& S, std::vector<double> W, std::size_t r1, std::size_t r2, double Z, char method) :
        S(S), W(W), r1(r1), r2(r2), Z(Z), method(method), pval(0) {}
    PermutationTest(const PermutationTest& p, RcppParallel::Split) :
        S(p.S), W(p.W), r1(p.r1), r2(p.r2), Z(p.Z), method(p.method), pval(0) {}

    // Call operator, compute test statistic for (end - begin) permuted samples
    void operator()(std::size_t begin, std::size_t end) {
        std::mt19937 prng(std::random_device{}());
        for (std::size_t i = begin; i < end; ++i) {
            double z = testStatistics<RcppParallel::RMatrix<double> >(S, W, r1, r2, prng, method)[2]; /** sc changed here **/
            pval += (z >= Z);
        }
    }
...
struct PermutationTestSeeded : public RcppParallel::Worker {
    ...
    // Constructors
    PermutationTestSeeded(const NumericMatrix& S, std::vector<double> W, std::size_t r1, std::size_t r2, double Z, char method,
                          const std::vector<std::vector<std::size_t> >& shuffles) :
        S(S), W(W), r1(r1), r2(r2), Z(Z), method(method), shuffles(shuffles), pval(0) {}
    PermutationTestSeeded(const PermutationTestSeeded& p, RcppParallel::Split) :
        S(p.S), W(p.W), r1(p.r1), r2(p.r2), Z(p.Z), method(p.method), shuffles(p.shuffles), pval(0) {}

    // Call operator, compute test statistic for (end - begin) permuted samples
    void operator()(std::size_t begin, std::size_t end) {
        for (std::size_t i = begin; i < end; ++i) {
            std::vector<std::size_t> shuffle = shuffles[i];
            double z = testStatistics<RcppParallel::RMatrix<double> >(S, W, r1, r2, shuffle, method)[2]; /** sc changed here **/
            pval += (z >= Z);
        }
    }
...
unsigned int permutationTestParallelSeeded(const NumericMatrix& S1,
    ...
    // Permute data 'nPermute' times and compute test statistic in each case
    PermutationTestSeeded pt(S, W, r1, r2, Z, method, shuffles); /** sc changed here **/
    parallelReduce(0, nPermute, pt);
    return pt.pval;
}
