...
std::vector<RTree> buildRangeTrees(const MatrixT& M,
    ...
    for (std::size_t i = 0; i < r1; ++i) {
        ...
        pts1.push_back(Point(getRow<MatrixT>(M, s[i]), (double) w[i])); /** sc changed here **/
    }

    // Extract points from second sample
    for (std::size_t i = 0; i < r2; ++i) {
        // pts2.push_back(Point(getRow<MatrixT>(M, s[i + r1]), 0));
        pts2.push_back(Point(getRow<MatrixT>(M, s[i + r1]), (double) w[i + r1])); /** sc changed here **/
    }
...
double rangeDistance(const RTree& rtree1,
                     ...
                     // const int n2,
                     const double w1,
                     const double w2, /** sc changed here BEGIN **/
    ...
    for (std::size_t i = 0; i < (1 << ndim); ++i) {
        ...
        std::vector<Point> vec1 = rtree1.pointsInRange(lowerLims, upperLims, strict, strict); /** sc changed here BEGIN **/
        std::vector<Point> vec2 = rtree2.pointsInRange(lowerLims, upperLims, strict, strict); 
        double res1 = 0.0;
        double res2 = 0.0;
        ...
        d = std::max(d, std::abs(res1 / w1 - res2 / w2)); /** sc changed here BEGIN **/
    }
    return d;
}
